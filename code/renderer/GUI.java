...
public abstract class GUI {
	...
	private void initialise() {
		...
		addLight.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				//adds a random lightsource between 1 and -1  the xyz values and a random color
				Renderer.directLightSources.add(new Vector3D(
		    			(float) (Math.random() - Math.random()),
		    			(float) (Math.random() - Math.random()),
		    			(float) (Math.random() - Math.random())));			
				Renderer.directLightColors.add(new Color((int) (Math.random()*100), (int) (Math.random()*100),(int) (Math.random()*100)));
		...
		removeLight.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				if(Renderer.directLightSources.size()>1) {
				Renderer.directLightSources.remove(Renderer.directLightSources.size()-1);			
				Renderer.directLightColors.remove(Renderer.directLightColors.size()-1);
				redraw();
			}}
		});
